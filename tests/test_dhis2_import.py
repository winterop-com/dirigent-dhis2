...
from dhis2server import BASE_URL, CONNECTION, json, summary
...
async def test_a_clean_import_reports_its_counts(ctx: FakeContext, dhis2: respx.Router) -> None:
    route = dhis2.post(IMPORT).mock(return_value=json(200, summary("SUCCESS", imported=12, updated=3)))
...
async def test_a_refused_import_answers_409_with_the_summary_and_is_rejected(
    ctx: FakeContext, dhis2: respx.Router
) -> None:
    ...
    dhis2.post(IMPORT).mock(return_value=json(409, summary("ERROR", ignored=1, conflicts=conflicts)))
...
async def test_a_summary_of_error_on_200_is_rejected_the_same_way(ctx: FakeContext, dhis2: respx.Router) -> None:
    conflicts = [{"object": "de9", "value": "data element not found"}]
    dhis2.post(IMPORT).mock(return_value=json(200, summary("ERROR", ignored=1, conflicts=conflicts)))
    with pytest.raises(BlockFailure) as refused:
        await call_block(Dhis2DataValueSetImportOperator(), {"connection": CONNECTION, "data_values": DOCUMENT}, ctx)
    assert refused.value.error_class is ErrorClass.REJECTED
    assert "data element not found" in refused.value.message
...
async def test_an_atomic_import_that_ignored_values_is_rejected(ctx: FakeContext, dhis2: respx.Router) -> None:
    dhis2.post(IMPORT).mock(return_value=json(409, summary("WARNING", imported=2, ignored=5)))
    with pytest.raises(BlockFailure) as refused:
        await call_block(Dhis2DataValueSetImportOperator(), {"connection": CONNECTION, "data_values": DOCUMENT}, ctx)
    assert refused.value.error_class is ErrorClass.REJECTED
...
async def test_a_tolerant_import_that_ignored_values_still_settles(ctx: FakeContext, dhis2: respx.Router) -> None:
    dhis2.post(IMPORT).mock(return_value=json(409, summary("WARNING", imported=2, ignored=5)))
    output = await call_block(
        Dhis2DataValueSetImportOperator(),
...
async def test_a_409_without_a_summary_is_an_ordinary_refusal(ctx: FakeContext, dhis2: respx.Router) -> None:
    dhis2.post(IMPORT).mock(return_value=json(409, {"httpStatus": "Conflict", "message": "Import already in progress"}))
    with pytest.raises(BlockFailure) as refused:
        await call_block(Dhis2DataValueSetImportOperator(), {"connection": CONNECTION, "data_values": DOCUMENT}, ctx)
    assert refused.value.error_class is ErrorClass.REJECTED
...
async def test_a_refused_transport_is_classified_by_its_status(ctx: FakeContext, dhis2: respx.Router) -> None:
    dhis2.post(IMPORT).mock(return_value=json(503, {}))
    with pytest.raises(BlockFailure) as refused:
        await call_block(Dhis2DataValueSetImportOperator(), {"connection": CONNECTION, "data_values": DOCUMENT}, ctx)
    assert refused.value.error_class is ErrorClass.TRANSIENT


async def test_an_import_reads_its_document_back_from_storage(local_ctx: FakeContext, dhis2: respx.Router) -> None:
    ...
    route = dhis2.post(IMPORT).mock(return_value=json(200, summary("SUCCESS", imported=1)))
    output = await call_block(
        Dhis2DataValueSetImportOperator(), {"connection": CONNECTION, "source_uri": uri}, local_ctx
    )
    assert output.model_dump()["status"] == "SUCCESS"
    assert jsonlib.loads(route.calls.last.request.content) == DOCUMENT


async def test_a_source_that_is_not_json_is_rejected(local_ctx: FakeContext, dhis2: respx.Router) -> None:
    uri = f"{local_ctx.scratch_uri}/dhis2/broken.json"
    local_ctx.storage.path_for(uri).parent.mkdir(parents=True, exist_ok=True)
    local_ctx.storage.path_for(uri).write_bytes(b"not json")
    dhis2.post(IMPORT).mock(return_value=json(200, summary("SUCCESS")))
    with pytest.raises(BlockFailure) as refused:
