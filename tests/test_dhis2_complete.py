...
async def test_a_completed_registration_ends_the_wait(ctx: FakeContext, dhis2: respx.Router) -> None:
    route = dhis2.get(REGISTRATIONS).mock(
        return_value=json(
            200,
            {
...
async def test_an_unclosed_window_is_not_yet(ctx: FakeContext, dhis2: respx.Router) -> None:
    dhis2.get(REGISTRATIONS).mock(return_value=json(200, {}))
...
async def test_a_reopened_registration_keeps_waiting(ctx: FakeContext, dhis2: respx.Router) -> None:
    dhis2.get(REGISTRATIONS).mock(
        return_value=json(200, {"completeDataSetRegistrations": [{"period": "2026Q1", "completed": False}]})
    )
    waited = await call_block(Dhis2DataSetCompleteSensor(), ASKED, ctx)
    assert isinstance(waited, NotYet)


async def test_an_old_instance_without_the_flag_counts_the_registration(ctx: FakeContext, dhis2: respx.Router) -> None:
    dhis2.get(REGISTRATIONS).mock(
        return_value=json(200, {"completeDataSetRegistrations": [{"period": "2026Q1", "date": "2026-04-02"}]})
    )
    output = await call_block(Dhis2DataSetCompleteSensor(), ASKED, ctx)
    assert isinstance(output, Dhis2DataSetCompleteOutput)
    assert output.completed_at == "2026-04-02"
...
async def test_a_refused_poke_is_classified_by_its_status(ctx: FakeContext, dhis2: respx.Router) -> None:
    dhis2.get(REGISTRATIONS).mock(return_value=json(500, {}))
