...
| [`dhis2-compose/`](#composed-with-the-engines-own-blocks) | A `dhis2.*` block beside one of the engine's own: a schema gate, a jq reshape, a fan-out over org units. |
...
| `dhis2-export-to-storage.yaml` | `dhis2-export-to-storage` | Stream an export to a storage URI with `save_to` instead of carrying it inline. |
...
| `dhis2-import-from-storage.yaml` | `dhis2-import-from-storage` | Export to storage and import back from it with `source_uri`. |
...
| `dhis2-analytics.yaml` | `dhis2-analytics` | The production-shaped nightly: sign-off gate, rebuild, export to storage. |
...
ends up checked, reshaped, or repeated once per organisation unit.
...
| `dhis2-export-per-org-unit.yaml` | `dhis2-export-per-org-unit` | One export per organisation unit with `for_each`, `items: continue` so one district's failure costs only that district. |
...
| `dhis2-org-units.yaml` | `dhis2-org-units` | A compact snapshot of every organisation unit, saved as an artifact. |
...
| `dhis2-export-data-values-csv.yaml` | `dhis2-export-data-values-csv` | A data value set as CSV rather than JSON -- the extension is the content negotiation -- streamed to storage. |
